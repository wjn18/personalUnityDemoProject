...
public class GuardHealthBarUI : MonoBehaviour
{
    [Header("Refs")]
    ...
    [Header("Target")]
    public GuardRuntime guard; // 绑定敌人
    ...
    {
        if ( guard == null)
            guard = GetComponentInParent<GuardRuntime>();

        if (guard != null)
        {
            guard.OnHPChanged += OnEnemyHPChanged;

            // 初始化
            SetHealth(guard.hp, guard.config.maxHP);
        }
    }
